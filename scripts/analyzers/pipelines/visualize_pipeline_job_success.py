"""
Visualize Pipeline Job Success Rates per Project

Creates one stacked bar chart per project showing job success rates for each issue/branch.
Shows percentage of successful build jobs and test jobs.
...
Output:
    - One PNG per project: visualizations/pipeline_job_success/{project}_job_success.png
    - Statistics CSV: pipeline_job_success_statistics.csv

Usage:
    python visualize_pipeline_job_success.py
"""
...
def get_job_success_rates(project_name, data_base_dir=None):
    ...
    for pipeline in pipelines:
        ...
        if build_status == 'success' and test_status == 'success':
            ...
            stats['build_only'] += 1
        else:  # Build nicht success
            stats['build_failed'] += 1

    # Calculate percentages based on total pipelines
    result = {}
...
def create_job_success_chart(project_label, job_stats, output_file):
    ...
    # Prepare data for 100% stacked bar chart (Pipeline-based)
    issues = []
    ...
    # Create stacked bars (from bottom to top) - 100% = alle Pipelines
    ...
    for i in range(len(issues)):
        percentages = [both_success_pct[i], build_only_pct[i], build_failed_pct[i]]
        bottoms = [0, both_success_pct[i], both_success_pct[i] + build_only_pct[i]]
        colors_text = ['white', 'white', '#34495E']  # White on green/orange, dark on gray
...
def main():
    """Generate job success rate visualizations for all projects"""

    print("="*80)
    print("PIPELINE-JOB ERFOLGSRATEN - GESTAPELTE SÄULENDIAGRAMME PRO PROJEKT")
    print("="*80)
    print()
    ...
    output_dir = os.path.join('..', '..', 'visualizations', 'pipeline_job_success')
    ...
    for project_id, project_name in projects:
        ...
        if job_stats:
            ...
            output_file = os.path.join(output_dir, f'{project_label.lower()}_job_success.png')
    ...
    stats_file = os.path.join(summary_dir, 'pipeline_job_success_statistics.csv')
